<template>
  <div class="auto-goods">
    <div class="rc-design-editor-component-title">商品</div>
    ...
    <el-form ref="form" :model="form" label-width="100px">
      ...
      <div class="controls-card add-good">
        <div class="imgLoad-style">
          ...
          <div v-if="form.goodSource == '1'">
            <el-form-item label="商品分组:" style="margin-top:20px;">
              <a
                @click="cardAddFun(true)"
                v-if="form.goodSource==='1'&&(goodsList&&goodsList.length===0)"
              >从商品分组中选择</a>
              <div
                style="display:flex"
                v-if="form.goodSource==='1'&&(goodsList&&goodsList.length>0)"
              >
                <div
                  class="zent-tag-style-outline"
                  style="border-color: rgba(51, 136, 255, 0.3); color: rgb(102, 102, 102); background: rgb(226, 243, 255);"
                >
                  <div class="zent-tag-content">
                    商品标签
                    |
                    {{form.goodSource==='1'&&goodsList[0].hasOwnProperty('CateName')?goodsList[0].CateName:goodsList[0].Name}}
                  </div>
                  <i class="zenticon zenticon-close zent-tag-close-btn"></i>
                </div>
                <a @click="cardAddFun(true)">修改</a>
              </div>
            </el-form-item>
            <el-form-item label="显示个数:" style="margin-top:20px;">
              <el-input-number
                style="width:95px"
                v-model="form.goodsMaxNum"
                @change="changeGoodsNum"
                :min="1"
                :max="50"
                :controls="false"
              ></el-input-number>
              <span style="margin-left:10px">最多显示50个</span>
            </el-form-item>
          </div>
        </div>
      </div>
    ...
    <goodsAuto ref="goodsAuto" @beforeClose="beforeClose" @sureGood="sureGood" :isGroup="isGroup"></goodsAuto>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeCheckbox() {
      console.log(this.form.showContent)
    ...
    sureGood(arr) {
      ...
      if (!this.isGroup) {
        ...
        this.form._data = this.form._data.concat(arrNew);
