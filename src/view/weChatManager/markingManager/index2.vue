    ...
    <el-table :data="tableData" style="width: 100%" v-loading="loading">
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scoped">
          <el-button type="text" v-if="scoped.row.Audit ==='1'" @click="audit(scoped.row)">取消审核</el-button>
          <el-button type="text" @click="audit(scoped.row)" v-else>审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    <audit :show="show" :showID = "showID" @auditFunction="auditFunction" @confirmEnd="confirmEnd"></audit>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async confirmEnd(val) {
      ...
      try {
        await getLists(
          { Action: "AuditLeader", OpenID: this.current_OpenID, Audit:this.current_Status},
          "MMemberOpera"
        );
