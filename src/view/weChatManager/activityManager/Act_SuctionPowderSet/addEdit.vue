    ...
    <el-form ref="form" :model="form" label-width="120px">
      ...
      <el-form-item label="活动类型">
        <el-select
          ...
          :disabled="disabled"
        >
          <el-option label="邀请关注" value="0"></el-option>
      ...
      <div v-if="acType == 0">
        ...
        <div v-for="(item, index) in form.GiveList" :key="index" class="addDiv">
          ...
          <el-form-item
            :key="index + 2"
            :prop="'GiveList.' + index + '.GiveCnt'"
            label="发放总量"
          >
            <el-input
              v-model="item.GiveCnt"
              class="inputwith"
          ...
          <el-form-item
            :key="index + 3"
            :prop="'GiveList.' + index + '.MeetCnt'"
            label="邀请人数"
          >
            <el-input
              v-model="item.MeetCnt"
              class="inputwith"
              onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
            ></el-input>
            人
          ...
          <el-form-item
            :key="index + 5"
            :prop="'GiveList.' + index + '.GiveScore'"
            label="赠送积分："
          >
            <el-input
              v-model="item.GiveScore"
              class="inputwith"
              onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
            ></el-input>
          </el-form-item>
          <el-form-item
            :key="index + 6"
            :prop="'GiveList.' + index + '.GiveInfo'"
            label="赠送电子券："
          >
            <el-input
              v-model="item.GiveInfo"
              readonly
              :disabled="true"
            ></el-input>
            <el-button
      ...
      <el-form-item label="背景图片" prop="Img">
        ...
        ></imgLoad>
        <span class="tips">
          <el-button @click="defaultImg" style="height:40px;margin-right:10px">下载范例</el-button>
          <span style="display:inline-block;width:60%">
            可将下载的范例图片直接上传使用，或是按图片格式设计新的页面上传。(图片格式要求：
            二维码尺寸：宽214像素，高214像素
  ...
  methods: {
    ...
    async preserveFun() {
      ...
      try {
        ...
        this.$router.push({
          path: "/weChat/manager/activity/Act_SuctionPowder",
        });
      } catch (error) {
    ...
    defaultImg() {
      // 用户关注（imgUrl2） 0 邀请充值（imgUrl3） 3  营销推广4（imgUrl）
          if(this.acType!=''){
               let url = "";
               if(this.acType == "0"){
                    url = GetBaseImgUrl() + "Admin/" + this.imgUrl2;
               }else if(this.acType == "3"){
                    url = GetBaseImgUrl() + "Admin/" + this.imgUrl3;
               }else if(this.acType == "4"){
                    url = GetBaseImgUrl() + "Admin/" + this.imgUrl;
               }
